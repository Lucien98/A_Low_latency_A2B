...
def out_impl(val, share_idx, output_idx):
    global LINES
    LINES.append(f"out {val} {output_idx}_{share_idx}")
...
def execute_circuit(ports, bit_defines, bit_names, spec):
    ...
    for output_idx, (output_shares_tuple) in enumerate(outputs):
        # port_name = output_port_names[output_idx]
        for share_idx, share_val in enumerate(output_shares_tuple):
            out_impl(share_val, share_idx, output_idx)    
    print("\n".join(LINES))
