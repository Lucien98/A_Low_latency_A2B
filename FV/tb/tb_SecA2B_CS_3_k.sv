module tb_SecA2B_CS_3_k;
    parameter k = 8;
    localparam RND_BITS = 3*k;
    localparam LATENCY = 2;

    reg clk;
    reg [3*k-1:0] ina;
    reg [RND_BITS-1:0] rnd;
    wire [3*k-1:0] outs, outc;

    SecA2B_CS_3_k #(.k(k)) dut (
        .clk(clk),
        .ina(ina),
        .rnd(rnd),
        .outs(outs),
    ...
    function [k-1:0] reconstruct;
        input [3*k-1:0] shares;
        integer b, s;
        reg [k-1:0] res;
        begin
            for (b = 0; b < k; b++) begin
                res[b] = 0;
                for (s = 0; s < 3; s++) res[b] = res[b] ^ shares[s*k + b];
            end
            reconstruct = res;
        end
    endfunction
    ...
    initial begin
        integer i, err = 0;
        reg [k-1:0] a0, a1, a2, x_plain, s_plain, c_plain, sum;
        $display("Testing SecA2B_CS_3_k (k=%0d)", k);
